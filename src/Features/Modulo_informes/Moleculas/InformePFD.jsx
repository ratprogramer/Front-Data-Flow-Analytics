...
const dataSB = selectedCards
  ...
  .map(card => ({
    ...
    const PDFDocument = () => (
        <Document>
            {/* Check if there are different types, and insert a new page when necessary */}
            {dataPP.length > 0 && (
            ...
            {dataPT.length > 0 && dataPP.length > 0 && (  // Nueva página si hay PP y PT
                <Page size="A4" style={styles.page}>
            ...
            {dataSB.length > 0 && (  // Nueva página si hay SB
                <Page size="A4" style={styles.page}>
